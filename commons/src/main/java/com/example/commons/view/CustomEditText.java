...
public class CustomEditText extends RelativeLayout implements View.OnClickListener, TextWatcher, View.OnFocusChangeListener {
    ...
    public CustomEditText(Context context, @Nullable AttributeSet attrs) {
        ...
        inputHeight = typedArray.getLayoutDimension(R.styleable.CustomInputEditText_inputHeight, LayoutParams.WRAP_CONTENT);

        inputHint = typedArray.getString(R.styleable.CustomInputEditText_inputHint) == null ? "请输入内容" : typedArray.getString(R.styleable.CustomInputEditText_inputHint);
        hintColor = typedArray.getColor(R.styleable.CustomInputEditText_hintColor, Color.RED);
        hintSize = px2sp(typedArray.getDimension(R.styleable.CustomInputEditText_hintSize, sp2px(hintSize)));
        drawable = typedArray.getResourceId(R.styleable.CustomInputEditText_drawable, -1);
        drawablePadding = typedArray.getDimensionPixelOffset(R.styleable.CustomInputEditText_drawablePadding, -1);
        inputContent = typedArray.getString(R.styleable.CustomInputEditText_inputContent);

        inputPadding = typedArray.getDimensionPixelOffset(R.styleable.CustomInputEditText_inputPadding, -1);
        inputPaddingTop = typedArray.getDimensionPixelOffset(R.styleable.CustomInputEditText_inputPaddingTop, -1);
        inputPaddingRight = typedArray.getDimensionPixelOffset(R.styleable.CustomInputEditText_inputPaddingRight, -1);
        inputPaddingBottom = typedArray.getDimensionPixelOffset(R.styleable.CustomInputEditText_inputPaddingBottom, -1);
        inputPaddingLeft = typedArray.getDimensionPixelOffset(R.styleable.CustomInputEditText_inputPaddingLeft, -1);

        inputBackground = typedArray.getDrawable(R.styleable.CustomInputEditText_inputBackground);
        inputSize = px2sp(typedArray.getDimension(R.styleable.CustomInputEditText_inputSize, sp2px(inputSize)));
        inputColor = typedArray.getColor(R.styleable.CustomInputEditText_inputColor, Color.BLACK);
        rightButton = typedArray.getResourceId(R.styleable.CustomInputEditText_rightButton, -1);
        btnType = typedArray.getInt(R.styleable.CustomInputEditText_btnType, 1);
        btnVisibility = typedArray.getInt(R.styleable.CustomInputEditText_btnVisibility, 1);
        clearContentsFocus = typedArray.getBoolean(R.styleable.CustomInputEditText_clearContentsFocus, true);
        inputType = typedArray.getInt(R.styleable.CustomInputEditText_inputType, 0);
        typedArray.recycle();
        initView();
    }
        ...
        switch (view.getId()) {
            case DELETE_ID:
                if (btnType==1){
                    ...
                    if (eyesClose){
                        delBtn.setImageResource(R.mipmap.conceal_on);
                        editText.setInputType(InputType.TYPE_TEXT_VARIATION_PASSWORD);
                        Editable editable= editText.getText();
                        Selection.setSelection(editable, editable.length());
                        eyesClose = false;
                    }else {
                        delBtn.setImageResource(R.mipmap.conceal_off);
                        editText.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
                        Editable editable= editText.getText();
                        Selection.setSelection(editable, editable.length());
                        eyesClose = true;
