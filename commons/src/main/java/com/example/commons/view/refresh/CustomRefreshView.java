package com.example.commons.view.refresh;
...
import com.example.commons.view.CustomTextView;

/**
 * @Author skygge.
 * @Date on 2019-08-21.
 * @Github https://github.com/javofxu
 * @Dec: 一个支持网络错误重试，无数据页（可自定义），无网络界面（可自定义）的上拉加载更多，下拉刷新控件
 * @version: ${VERSION}.
 * @Update :
 */
public class CustomRefreshView extends FrameLayout implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private void setupSwipeRecyclerView() {
        ...
        View view = LayoutInflater.from(getContext()).inflate(R.layout.layout_custom_refresh, this);
     ...
     * @param s : 文案显示
     */
    public void setEmptyView(String s, int img) {
        ...
        if (mEmptyView == null) {
            ...
            mEmptyImg= mEmptyView.findViewById(R.id.module_base_id_empty_img);
            mEmptyText = mEmptyView.findViewById(R.id.module_base_empty_text);
        }
        blankView.addView(mEmptyView);
        if (img>0){
            mEmptyImg.setImageResource(img);
        }else {
            mEmptyImg.setImageResource(R.mipmap.icon_empty);
        }
